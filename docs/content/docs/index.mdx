...
- **Composable store backends** -- Three backends implement the same composite `store.Store` interface: `memory` (testing), `postgres` (pgx connection pool), and `grovestore` (Grove ORM). All support Migrate, Ping, and Close lifecycle methods.
...
<Cards>
  ...
  <Card title="Stores" href="/docs/stores/memory" description="Configure memory, PostgreSQL, or Grove store backends." />
</Cards>
