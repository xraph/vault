...
Alternatively, use the Grove ORM backend if your application already uses Grove:

```go
import grovestore "github.com/xraph/vault/store/grovestore"

groveStore := grovestore.New(db) // db is a *grove.DB
if err := groveStore.Migrate(ctx); err != nil {
    log.Fatal(err)
}
```
