...
| `store/grovestore` | `github.com/xraph/vault/store/grovestore` | `Store`, `StoreOption` | `New()`, `WithLogger()` | PostgreSQL backend using Grove ORM |
...
PostgreSQL via Grove ORM. Uses 11 model structs and `CreateTable` migrations. See [Grove ORM Store](/docs/stores/grove).
