...
func (s *Store) SetConfig(ctx context.Context, e *cfgpkg.Entry) error {
	...
	err = tx.NewSelect().Model((*ConfigModel)(nil)).
		Column("version").
		Where("key = ?", e.Key).
	...
	if _, err := tx.NewInsert().Model(vm).Exec(ctx); err != nil {
		return err
	}
...
func (s *Store) DeleteConfig(ctx context.Context, key, appID string) error {
	...
	res, err := tx.NewDelete().Model((*ConfigModel)(nil)).
		Where("key = ?", key).
		Where("app_id = ?", appID).
		Exec(ctx)
	if err != nil {
	...
	if _, err := tx.NewDelete().Model((*ConfigVersionModel)(nil)).
