...
func (s *Store) DeleteFlagDefinition(ctx context.Context, key, appID string) error {
	...
	res, err := tx.NewDelete().Model((*FlagModel)(nil)).
		Where("key = ?", key).
		Where("app_id = ?", appID).
		Exec(ctx)
	if err != nil {
	...
	if _, err := tx.NewDelete().Model((*FlagRuleModel)(nil)).
		Where("flag_key = ?", key).
		Where("app_id = ?", appID).
		Exec(ctx); err != nil {
		return err
	}
	if _, err := tx.NewDelete().Model((*FlagOverrideModel)(nil)).
		Where("flag_key = ?", key).
		Where("app_id = ?", appID).
		Exec(ctx); err != nil {
		return err
...
func (s *Store) SetFlagTenantOverride(ctx context.Context, key, appID, tenantID string, value any) error {
	...
	_, err = s.db.NewInsert().Model(m).
		On("CONFLICT (flag_key, app_id, tenant_id) DO UPDATE").
...
func (s *Store) DeleteFlagTenantOverride(ctx context.Context, key, appID, tenantID string) error {
	res, err := s.db.NewDelete().Model((*FlagOverrideModel)(nil)).
		Where("flag_key = ?", key).
		Where("app_id = ?", appID).
		Where("tenant_id = ?", tenantID).
		Exec(ctx)
